...
public class GridBuildingSystem : MonoBehaviour
{
    ...
    [SerializeField] private int heigth = 10;               // 그리드 세로 크기
    [SerializeField] private float cellSize = 1;            // 각 셀의 크기

    [SerializeField] private GameObject cellPrefab;                 // 셀 프리펩
    ...
    [SerializeField] private Grid grid;                 // 그리드 선운 후 받아온다
    ...
    private void CreateGrid()
    {
        ...
        cells = new GridCell[width, heigth];                            // 그리드 가로 세로 칸수 설정
        Vector3 gridCenter = playerController.transform.position;       // 플레이어 중심으로 그리드 생성 (원하는 위치로 변경가능)
        gridCenter.y = 0;
        transform.position = gridCenter - new Vector3(width * cellSize / 2.0f, 0, heigth * cellSize / 2);   // 그리드 중심으로 이동 시킨다

        for (int x = 0; x < width; x++)
        {
            for (int z = 0; z < heigth; z++)
            {
                Vector3Int cellPosition = new Vector3Int(x, 0, z);                  // (0,0,0) (1,0,0), (2,0,0) ~ (10,0,10) for문 돌리면서 배치 
    ...
    private void HighlightCell(Vector3Int gridPosition)
    {
        for (int x = 0; x < width; x++)
        {
            for (int z = 0; z < heigth; z++)
            {
                // Cell 2차원 배열에 선언되어있는 게임 오브젝트를 가져온다
                GameObject cellObjcet = cells[x, z].Building != null ? cells[x, z].Building : transform.GetChild(x * heigth + z).gameObject;
                cellObjcet.GetComponent<Renderer>().material.color = Color.white;
            }
        }

        GridCell cell = cells[gridPosition.x, gridPosition.z];
        GameObject highlightObject =
            cell.Building != null ? cell.Building : transform.GetChild(gridPosition.x * heigth + gridPosition.z).gameObject;
